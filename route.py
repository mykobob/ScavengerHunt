class Route:
    ...
    def calculate_length(self):
        return float(sum(x.nbors[y] for x,y in zip(self.locations_order, self.locations_order[1:])))
    ...
    def __str__(self):
        return "Length: {}. {}".format(str(round(self.length,2)), " -> ".join([str(loc) for loc in self.locations_order]))
